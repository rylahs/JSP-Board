	...
	<div class="container">
		<div class="row">
			<table class="table table-hover table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					<%
						...
						int searchTotalCount = bbsDAO.getArticleSearchCount(searchCategory, searchString);
						int boardPerPage = searchNum;
						
						if (boardPerPage < 2)
							boardPerPage = 2;
						int startPage = 1;
						int endPage = (int)Math.ceil((double)searchTotalCount / boardPerPage);
								
						System.out.println("페이지 숫자 : " + pageNumber);
						
						
						
						ArrayList<Bbs> list = bbsDAO.getListString(pageNumber, boardPerPage, searchCategory, searchString);
			...
			%>
				...
				<a href="searchBbsResult.jsp?pageNumber=<%=1 %>&searchCategory=<%=searchCategory %>&searchString=<%=searchString %>&searchNum=<%=boardPerPage %>" class="btn btn-default btn-arraw-left"><%="<<" %></a>
			<%
				}
			%>
			<%
				if (pageNumber != startPage) {
			%>
				<a href="searchBbsResult.jsp?pageNumber=<%=pageNumber - 1 %>&searchCategory=<%=searchCategory %>&searchString=<%=searchString %>&searchNum=<%=boardPerPage %>" class="btn btn-default btn-arraw-left"><%="<" %></a>
			<%
				}
				...
				for (int i = startIdx; i <= endIdx; i++){
					if (i == pageNumber) {
				%>
					<a href="searchBbsResult.jsp?pageNumber=<%=i %>&searchCategory=<%=searchCategory %>&searchString=<%=searchString %>&searchNum=<%=boardPerPage %>" class="btn btn-default active"><%=i %></a>
				<%
					} else {
				%>
					<a href="searchBbsResult.jsp?pageNumber=<%=i %>&searchCategory=<%=searchCategory %>&searchString=<%=searchString %>&searchNum=<%=boardPerPage %>" class="btn btn-default"><%=i %></a>
				<%		
					}
				%>
			<%
			...
			%>
				<a href="searchBbsResult.jsp?pageNumber=<%=pageNumber + 1 %>&searchCategory=<%=searchCategory %>&searchString=<%=searchString %>&searchNum=<%=boardPerPage %>" class="btn btn-default btn-arraw-right"><%=">" %></a>
			...
			<%
				if(pageNumber != endPage) {
			%>
				<a href="searchBbsResult.jsp?pageNumber=<%=endPage %>&searchCategory=<%=searchCategory %>&searchString=<%=searchString %>&searchNum=<%=boardPerPage %> " class="btn btn-default btn-arraw-right"><%=">>" %></a>
			<%
				}
			%>
